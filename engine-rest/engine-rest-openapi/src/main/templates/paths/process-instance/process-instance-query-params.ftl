...
<@lib.parameter name="processDefinitionKeyNotIn"
    location="query"
    type="string"
    description="Exclude instances by a comma-separated list of process definition keys. A process instance must not have one of the given process definition keys."/>
...
<@lib.parameter name="variables"
    location="query"
    type="string"
    description="Only include process instances that have variables with certain values. Variable filtering expressions are comma-separated and are structured as follows:
